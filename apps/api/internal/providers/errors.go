...
func (e *BoundaryError) PresentationMessage() string { return e.Message }

func NewUnknownProviderError(providerID ProviderID) error {
	return &BoundaryError{Category: CategoryUnknownProvider, Message: fmt.Sprintf("Provider %q is not registered.", providerID), cause: ErrUnknownProvider}
}
func NewUnknownModelError(providerID ProviderID, modelID ModelID) error {
	return &BoundaryError{Category: CategoryUnknownModel, Message: fmt.Sprintf("Model %q is not registered for provider %q.", modelID, providerID), cause: ErrUnknownModel}
}
func NewUnsupportedCapabilityError(providerID ProviderID, modelID ModelID, capability Capability) error {
	return &BoundaryError{Category: CategoryUnsupportedCapability, Message: fmt.Sprintf("Model %q for provider %q does not support capability %q.", modelID, providerID, capability), cause: ErrUnsupportedCapability}
}
func NewUnavailableError(cause error) error {
	return &BoundaryError{Category: CategoryProviderUnavailable, Message: "The selected provider is unavailable.", cause: errors.Join(ErrProviderUnavailable, cause)}
}
func NewExecutionError(cause error) error {
	return &BoundaryError{Category: CategoryProviderExecution, Message: "The provider could not complete text generation.", cause: errors.Join(ErrProviderExecution, cause)}
}
...
func NewDuplicateRegistrationError(providerID ProviderID) error {
	return &BoundaryError{Category: CategoryDuplicateRegistration, Message: fmt.Sprintf("Provider %q is already registered.", providerID), cause: ErrDuplicateRegistration}
}
func NewSpeechInputTooLongError() error {
	return &BoundaryError{Category: CategorySpeechInputTooLong, Message: "The speech narration exceeds the selected provider's input limit.", cause: errors.Join(ErrInvalidRequest, ErrSpeechInputTooLong)}
}
