...
// Code identifies stable generation-engine failure categories.
type Code string
...
// Error is a presentation-safe generation-engine failure.
type Error struct {
	Code    Code
	Message string
	cause   error
...
func (e *Error) PresentationMessage() string {
	return e.Message
}
...
func newProviderFailedError(cause error) error {
	return &Error{
		...
		Message: "Proposal generation could not be completed.",
...
func newInvalidOutputError(cause error) error {
	...
	return &Error{
		...
		Message: "The model output did not satisfy the proposal contract.",
...
func mapProviderError(err error) error {
	...
	case errors.Is(err, providers.ErrProviderExecution):
