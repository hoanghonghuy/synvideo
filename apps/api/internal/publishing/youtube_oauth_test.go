...
func TestYouTubeOAuthRejectsIncompleteConfigAndEmptyState(t *testing.T) {
	if _, err := NewYouTubeOAuth(YouTubeOAuthConfig{}, nil); !errors.Is(err, ErrOAuthConfiguration) {
	...
	if _, err := oauth.AuthorizationURL(" "); !errors.Is(err, ErrOAuthConfiguration) {
