...
func TestHandlerReusesDurablyIngestedAssetWithoutRegeneration(t *testing.T) {
	...
	if runtime.resolveCalls != 0 {
		t.Fatalf("recovery regenerated image")
	}
	if store.storeCalls != 0 {
		t.Fatalf("recovery duplicated media asset")
	}
	if binder.assignCalls != 1 {
		t.Fatalf("expected one assignment, got %d", binder.assignCalls)
	}
	var result Result
	if err := json.Unmarshal(resultRaw, &result); err != nil {
		t.Fatal(err)
	}
	if result.MediaAssetID != assetID || !result.AssignedPrimaryVisual {
...
func TestHandlerDuplicateDeliveryDoesNotDuplicateCurrentAssignment(t *testing.T) {
	...
	if binder.assignCalls != 0 {
		t.Fatalf("duplicate delivery wrote assignment history")
	}
}
...
func TestHandlerGeneratesAndStoresSafeProvenance(t *testing.T) {
	...
	if gen.calls != 1 || store.storeCalls != 1 {
		t.Fatalf("generation/store calls = %d/%d", gen.calls, store.storeCalls)
	}
	var metadata map[string]any
	if err := json.Unmarshal(store.lastInput.Metadata, &metadata); err != nil {
		t.Fatal(err)
	}
	if metadata["job_id"] != jobID.String() || metadata["provider_id"] != "openai" || metadata["model_id"] != "image-1" {
		t.Fatalf("unsafe/missing provenance: %v", metadata)
	}
	for _, forbidden := range []string{"provider_url", "base_url", "credential", "external_model_id", "raw_response"} {
		if _, ok := metadata[forbidden]; ok {
			t.Fatalf("forbidden provenance key %q", forbidden)
		}
	}
}
...
func (f *fakeGeneratedAssetStore) Store(_ context.Context, principal project.Principal, projectID uuid.UUID, input mediaasset.CreateInput) (mediaasset.MediaAsset, error) {
	...
	if len(data) == 0 {
		return mediaasset.MediaAsset{}, errors.New("empty")
	}
...
func (f *countingImageGenerator) GenerateImage(context.Context, providers.ImageGenerationRequest) (providers.ImageGenerationResponse, error) {
	f.calls++
	return f.response, f.err
}

type fakeBinder struct {
	current     scenemedia.Binding
	currentErr  error
	assignCalls int
}

func (f *fakeBinder) GetCurrent(context.Context, project.Principal, uuid.UUID, int, string) (scenemedia.Binding, error) {
	return f.current, f.currentErr
}
func (f *fakeBinder) AssignPrimaryVisual(_ context.Context, _ project.Principal, _ uuid.UUID, _ int, _ string, assetID uuid.UUID) (scenemedia.Binding, error) {
	f.assignCalls++
	f.current = scenemedia.Binding{AssetID: assetID}
	f.currentErr = nil
