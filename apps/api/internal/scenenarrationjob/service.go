...
var (
	...
	ErrEmptyNarration            = errors.New("scene narration is empty")
...
type TTSProviderRuntime interface {
	ResolveSpeechSynthesizer(context.Context, uuid.UUID, providers.ProviderID, providers.ModelID) (providers.SpeechSynthesizer, error)
}

type JobRepository interface {
	Enqueue(context.Context, jobs.EnqueueInput) (jobs.Job, error)
	GetByIDForProject(context.Context, uuid.UUID, uuid.UUID, uuid.UUID) (jobs.Job, error)
}
...
func NewService(runtime TTSProviderRuntime, jobsRepo JobRepository, projects ProjectRepository, plans ScenePlanRepository) *Service {
	return &Service{
		runtime:  runtime,
		jobs:     jobsRepo,
		projects: projects,
...
func (s *Service) CreateGeneration(ctx context.Context, principal project.Principal, projectID uuid.UUID, planVersion int, sceneKey string, input CreateGenerationInput) (JobView, error) {
	...
	if input.ProviderID == "" || input.ModelID == "" || input.VoiceID == "" {
	...
	_, err := s.projects.Get(ctx, principal.OwnerID, projectID)
	if err != nil {
	...
	if strings.TrimSpace(scene.Narration) == "" {
		return JobView{}, ErrEmptyNarration
	}

	if s.runtime == nil {
		return JobView{}, ErrProviderUnavailable
	}
	synthesizer, err := s.runtime.ResolveSpeechSynthesizer(ctx, principal.OwnerID, providers.ProviderID(input.ProviderID), providers.ModelID(input.ModelID))
	...
	payload := Payload{
		...
		AssignCurrent:    input.AssignCurrent,
	}
	payloadBytes, err := json.Marshal(payload)
	if err != nil {
		return JobView{}, fmt.Errorf("marshal payload: %w", err)
	}
	...
	if err != nil {
		...
		return JobView{}, fmt.Errorf("enqueue narration generation job: %w", err)
	}
...
func validateExistingJob(job jobs.Job, planVersion int, sceneKey string, input CreateGenerationInput) error {
	...
	if payload.SchemaVersion != SchemaVersion ||
		payload.ScenePlanVersion != planVersion ||
		payload.SceneKey != sceneKey ||
		payload.ProviderID != input.ProviderID ||
		payload.ModelID != input.ModelID ||
		payload.VoiceID != input.VoiceID ||
		payload.Format != format ||
		payload.AssignCurrent != input.AssignCurrent {
		return ErrGenerationRequestConflict
	}
	return nil
}
...
func ToJobView(job jobs.Job) JobView {
	...
	if len(job.Result) > 0 {
		...
		if json.Unmarshal(job.Result, &result) == nil && result.MediaAssetID != uuid.Nil {
			...
			view.DurationSeconds = result.DurationSeconds
			view.AssignedNarration = result.AssignedNarration
		}
	}
	return view
}
