...
func TestListCreativeProposalsEndpoint(t *testing.T) {
	...
	server := New(config.Config{Environment: "test"}, nil, nil, nil, service, fixedResolver{ownerID: ownerID})
...
func TestPutCreativeProposalEndpointRejectsServerControlledFields(t *testing.T) {
	...
	server := New(config.Config{Environment: "test"}, nil, nil, nil, fakeCreativeProposalService{}, fixedResolver{ownerID: ownerID})
