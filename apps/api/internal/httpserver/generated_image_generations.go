...
func (h generatedImageGenerationHandler) create(w http.ResponseWriter, r *http.Request) {
	principal, ok := h.resolvePrincipal(w, r); if !ok { return }
	projectID, ok := parseProjectID(w, r); if !ok { return }
	version, err := strconv.Atoi(r.PathValue("version")); if err != nil || version < 1 { writeAPIError(w, project.ValidationError{Fields: map[string]string{"version":"positive_integer"}}); return }
	sceneKey := r.PathValue("scene_key"); if sceneKey == "" { writeAPIError(w, project.ValidationError{Fields: map[string]string{"scene_key":"required"}}); return }
	var req createGeneratedImageRequest
	decoder := json.NewDecoder(r.Body); decoder.DisallowUnknownFields()
	if err := decoder.Decode(&req); err != nil { writeAPIError(w, project.ValidationError{Fields: map[string]string{"body":"invalid_json"}}); return }
	requestID, err := uuid.Parse(req.RequestID); if err != nil || requestID == uuid.Nil { writeAPIError(w, project.ValidationError{Fields: map[string]string{"request_id":"invalid_uuid"}}); return }
	if req.ProviderID == "" || req.ModelID == "" { fields := map[string]string{}; if req.ProviderID == "" { fields["provider_id"]="required" }; if req.ModelID == "" { fields["model_id"]="required" }; writeAPIError(w, project.ValidationError{Fields:fields}); return }
	...
	if err != nil { writeGeneratedImageAPIError(w, err); return }
...
func (h generatedImageGenerationHandler) get(w http.ResponseWriter, r *http.Request) {
	principal, ok := h.resolvePrincipal(w, r); if !ok { return }
	projectID, ok := parseProjectID(w, r); if !ok { return }
	jobID, err := uuid.Parse(r.PathValue("job_id")); if err != nil || jobID == uuid.Nil { writeAPIError(w, project.ValidationError{Fields:map[string]string{"job_id":"invalid_uuid"}}); return }
	view, err := h.service.GetGeneration(r.Context(), principal, projectID, jobID); if err != nil { writeGeneratedImageAPIError(w, err); return }
...
func (h generatedImageGenerationHandler) resolvePrincipal(w http.ResponseWriter, r *http.Request) (project.Principal, bool) {
	if h.actorResolver == nil { writeAPIError(w, project.ErrUnauthenticated); return project.Principal{}, false }
	principal, err := h.actorResolver.Resolve(r); if err != nil { writeAPIError(w, project.ErrUnauthenticated); return project.Principal{}, false }
...
func toGeneratedImageJobResponse(view generatedimagejob.JobView) generatedImageJobResponse {
	...
	if view.MediaAssetID != nil { value := view.MediaAssetID.String(); response.MediaAssetID = &value }
	return response
}
