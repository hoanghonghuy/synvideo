...
func TestGetTextGenerationOptionsEndpoint(t *testing.T) {
	...
	t.Run("authenticated owner returns scoped options", func(t *testing.T) {
		...
		server := New(config.Config{Environment: "test"}, nil, nil, nil, nil, nil, nil, serviceWithOwner, nil, nil, nil, resolver)
	...
	t.Run("unauthenticated request returns 401 and does not expose global options", func(t *testing.T) {
		...
		server := New(config.Config{Environment: config.EnvironmentProduction}, nil, nil, nil, nil, nil, nil, service, nil, nil, nil, resolver)
		req := httptest.NewRequest(http.MethodGet, "/api/v1/ai/text-generation-options", nil)
		rec := httptest.NewRecorder()
		server.Handler.ServeHTTP(rec, req)
...
func TestCreateProposalGenerationEndpoint(t *testing.T) {
	...
	t.Run("202 Accepted on success", func(t *testing.T) {
		...
		server := New(config.Config{Environment: "test"}, nil, nil, nil, nil, nil, nil, service, nil, nil, nil, fixedResolver{ownerID: ownerID})
