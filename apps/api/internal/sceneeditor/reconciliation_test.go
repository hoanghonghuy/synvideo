...
func TestReconciliationPreservesLocalIdentityAndPresentation(t *testing.T) {
	...
	if err != nil { t.Fatalf("PreviewReconciliation: %v", err) }
	if preview.Ambiguous || len(preview.Changes) != 1 || !preview.Changes[0].PreservesEdits { t.Fatalf("preview=%+v", preview) }

	updated, err := ApplyReconciliation(doc, candidate, 1, now.Add(time.Minute))
	if err != nil { t.Fatalf("ApplyReconciliation: %v", err) }
	if updated.ID != doc.ID || updated.Scenes[0].ID != sceneID { t.Fatal("composition and local scene identity must remain stable") }
	if updated.Revision != 2 || updated.ScenePlanVersion != 2 { t.Fatalf("revision=%d plan=%d", updated.Revision, updated.ScenePlanVersion) }
	if updated.Scenes[0].Narration.LineageID != newNarration.LineageID { t.Fatal("new narration lineage was not applied") }
	if updated.Scenes[0].Notes != "creator note" || updated.Scenes[0].VisualTreatment.Scale != 1.25 { t.Fatal("unrelated creator presentation edits were not preserved") }
}
...
func TestReconciliationRejectsMissingOrAmbiguousSceneKey(t *testing.T) {
	...
	for name, candidate := range map[string]ReconcileCandidate{
		...
		t.Run(name, func(t *testing.T) {
			preview, err := PreviewReconciliation(doc, candidate)
			if err != nil { t.Fatalf("PreviewReconciliation: %v", err) }
			if !preview.Ambiguous { t.Fatalf("preview=%+v want ambiguous", preview) }
			if _, err := ApplyReconciliation(doc, candidate, 1, now); !errors.Is(err, ErrAmbiguousMapping) { t.Fatalf("err=%v want ambiguous mapping", err) }
		})
	}
}
...
func TestReconciliationRejectsStaleWriter(t *testing.T) {
	...
	if _, err := ApplyReconciliation(doc, candidate, 0, time.Now().UTC()); !errors.Is(err, ErrConflict) { t.Fatalf("err=%v want conflict", err) }
}
